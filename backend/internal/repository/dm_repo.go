...
func (r *DMRepo) FetchLastMessages(ctx context.Context, convoIDs []string) (map[string]models.Message, error) {
	...
	rows, err := r.db.Query(ctx,
		`SELECT DISTINCT ON (m.conversation_id)
		        m.id, m.conversation_id, m.author_id, m.system_type, m.content, m.created_at, m.forwarded_message_id,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&msg.ID, &convID, &msg.AuthorID, &msg.SystemType, &msg.Content, &msg.CreatedAt, &msg.ForwardedMessageID,
